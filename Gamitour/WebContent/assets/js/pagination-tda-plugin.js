(function($) {
	...
	$.fn.paginationTdA = function(options) {
		...
		var numberOfTds = idTable.find("tbody").eq(0).find("tr").eq(0).find(
				"td").length;
		createTableFooterPagination(idTable, numberOfTds, last);

		idTable.find("tbody").eq(0).find(".elementToPaginate").each(
				function(i) {
					$(this).attr("data-number", (i + 1));
					// Ocultamos solo los que no sean inferiores o iguales al
					// elemPerPage (para visualizar los primeros)
					if ((i + 1) > elemPerPage) {
						$(this).hide();
					}
				});
		...
		$("body").on(
				...
				function(e) {
					e.preventDefault();
					idTable.find("tbody").eq(0).find(".elementToPaginate")
							.hide();
					...
					for (var i = startIn; i <= stopIn; i++) {
						idTable.find("tbody").eq(0).find(
								".elementToPaginate[data-number='" + i + "']")
								.fadeIn();
					}
		...
		/*
		 * Al clicar en 'primero' emulamos el algoritmo con nClicked = 1 (como
		 * si hubieramos clicado en 1)
		 */
		$("body").on(
				"click",
				".paginationInit",
		...
		/*
		 * Al clicar en 'ultimo' emulamos el algoritmo con nClicked = last (como
		 * si hubieramos clicado en el ultimo)
		 */
		$("body").on(
				"click",
				".paginationEnd",
