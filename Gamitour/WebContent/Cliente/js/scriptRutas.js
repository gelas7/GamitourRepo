window.onload = function() {
    ...
    function cargarItinerarios(directionsService, directionsDisplay) {
        ...
        xhr.open('GET', 'json/itinerario.json', true); //Abro el flujo de Json
        xhr.send();

        xhr.onload = function() {
            if (this.status == 200) {
                itinerarios = JSON.parse(this.responseText);
            }

            document.getElementById('contenedorItinerarios').innerHTML = "";

            for (var j = 0; j < itinerarios.length; j++) { //Recorro los itinerarios
                waypts = [];
                origen = itinerarios[j].paradas[0].ubicacion; //1ª parada
                destino = itinerarios[j].paradas[itinerarios[j].paradas.length - 1].ubicacion; //Ultima parada

                var divMapa = `
                <div id="mapaItinerario${j}" class="mapItinerario">
                </div>
                <button id="tituloItinerario${j}" class="botonesItinerario" value=${j}>${itinerarios[j].nombre}</button> 
                <div id="direccionesItinerario${j}" class="directions">
                </div>

                `;
                document.getElementById("contenedorItinerarios").innerHTML += divMapa; //Inserto div mapa


                if (itinerarios[j].paradas.length > 2) { //Resto de paradas
                    for (var i = 1; i < itinerarios[j].paradas.length - 1; i++) {
                        waypts.push({
                            location: itinerarios[j].paradas[i].ubicacion, //Aqui se añaden los nombre de ubicaciones intermedias
                            stopover: true
                        });
                    }
                }
    ...
    function cargarParadas() {
        ...
        var paradas = itinerarios[numItinerario].paradas;
        document.getElementById('contenedorParadas').innerHTML = "";

        for (let parada in paradas) { //Recorro las paradas para pintar sus divs con titulo
            divMapa = `
    ...
    function pintarParadas(paradas) { //Le paso el array de paradas 
        ...
        for (let parada in paradas) { //Recorro las paradas
            var mapaActual = "mapaParada" + contador; //Construyo el id del mapa actual
            window[mapaActual] = new google.maps.Map(document.getElementById(mapaActual), {
                center: {
